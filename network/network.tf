...
resource "aws_vpc_peering_connection" "default" {
  ...
  vpc_id      = "${module.mumbai_network.vpc.id}"
  peer_vpc_id = "${module.singapore_network.vpc.id}"
...
resource "aws_route" "mumbai_to_singapore" {
  provider                  = "aws.ap-south-1"
  route_table_id            = "${module.mumbai_network.route_table.id}"
...
resource "aws_route" "singapore_to_mumbai" {
  provider                  = "aws.ap-southeast-1"
  route_table_id            = "${module.singapore_network.route_table.id}"
